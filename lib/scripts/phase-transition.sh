...
PROJECT_ROOT="$(pwd)"  # must be run from the consumer project root
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLANS="$PROJECT_ROOT/plans"
ARCHIVE="$PLANS/_archive/phase-$FROM"
TEMPLATE="${SCRIPT_DIR}/../templates/phase-index-template.md"  # internal script asset — exempt from [process-type].[file-type]-template.md convention
...
if [[ ! -f "$TEMPLATE" ]]; then
  say "   ⚠  Template not found at lib/templates/phase-index-template.md — skipping"
else
  if $DRY_RUN; then
