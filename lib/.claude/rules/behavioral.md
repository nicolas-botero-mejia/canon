...
For background → `system-principles.md (framework wiki, in node_modules)`
...
### 9. Template Coverage
Every file type must have a template before more files of that type are created. Template names follow `[process-type].[file-type]-template.md`. See `templates/template-index.md (framework templates, in node_modules)` for the full map and destination rules.

### 10. Meta-Doc Currency
`system-architecture.md (framework wiki, in node_modules)` and `system-operations.md (framework wiki, in node_modules)` are the system's source of truth — not regular wiki files. Any structural change (new folder, naming convention change, new template type, new process type, new behavioral rule) requires updating both before the session closes.
...
|--------|----------|-----------------|---------|
| **Project-local** | `[project-root]/.claude/skills/` | `/skill-name` (no prefix) | `/activity-conclude poc`, `/activity-conclude session`, `/wiki-manage`, `/activity-new poc`, `/activity-new session`, `/activity-new addendum`, `/conclusions-review`, `/knowledge-audit`, `/activity-update`, `/activity-deprecate`, `/activity-migrate`, `/phase-new`, `/phase-update`, `/phase-deprecate`, `/phase-reorder`, `/phase-conclude`, `/signal` |
