...
## Project layer — generated from frontmatter

These folders ship empty and are populated per engagement. Project files carry `description:` (and optionally `key_facts:` + `questions:`) frontmatter; `canon index` projects them into the marked section below (ADR-021). Files without a description stay unlisted until you add one — or hand-write an entry *outside* the markers (hand-written entries are never touched).

- `wiki/standards/` → industry / domain reference
- `wiki/project/` → client-specific decisions (what was chosen)
- `wiki/client/` → org knowledge (no frontmatter — ADR-012; indexed by H1 title)
- `wiki/user/` → end-user research (no frontmatter — ADR-012; indexed by H1 title)
...
- `raw/` → received source materials (immutable; not indexed)
- `tmp/` → transient working files (each carries a `Closes when:` condition; not committed)

<!-- canon:project-layer:begin — generated by `canon index` from file frontmatter; entries between markers are overwritten on regeneration -->

_No indexed project files yet — add `description:` frontmatter to files in the project dirs and re-run `canon index`._

<!-- canon:project-layer:end -->
