...
For background → `wiki/meta/prompting-workflow.md`
For system maintenance rules → `wiki/meta/system-maintenance.md`
...
### 9. Template Coverage
Every file type must have a template before more files of that type are created. Template names follow `[process-type].[file-type]-template.md`. See `wiki/meta/templates/TEMPLATE_MAP.md` for the full map and destination rules.

### 10. Meta-Doc Currency
`wiki/meta/architecture.md` and `wiki/meta/system-maintenance.md` are the system's source of truth — not regular wiki files. Any structural change (new folder, naming convention change, new template type, new process type, new behavioral rule) requires updating both before the session closes.
...
Do not close a session that adds or modifies governance mechanisms without running both levels. See `wiki/meta/architecture.md §7`.
