...
| `poc.results-template.md` | `findings/` | `phase-NN-poc-NN-[name]-results.md` | During/after a POC — structured execution log, raw observations, hypothesis resolution |
...
| `research.results-template.md` | `findings/` | `phase-NN-research-[topic]-results.md` | After executing a research plan — what was found, solved vs. gaps, sources |
...
| Process | Has plan? | Has results? | Has conclusions? | Notes |
|---------|-----------|-------------|-----------------|-------|
| `poc` | ✅ Required | ✅ Required | ✅ Required | Full lifecycle, heavy structure |
| `addendum` | ✅ Required | ✅ Required | ✅ Required | Extends any closed parent document (POC, research, or session). Numbered per parent (addendum-01, -02, …). New hypotheses only. `[parent-identifier]` in filename = parent's own ID segment. |
| `signal` | ❌ | ✅ Required | ❌ | Assessment artifact only. No parent doc. Routes to addendum, POC, research plan, or backlog. |
| `research` | ✅ Required | ✅ Required | ⚠️ Only if closes a decision | Lighter plan than POC |
| `session` | ✅ Required (in plans/) | ✅ field-notes (human) + results (AI, from transcript) | ✅ Required | Two findings types per session |
| `handoff` | ❌ | ✅ in findings/ — use `handoff.results-template.md` | ❌ | Transitional doc between phases/POCs |
...
3. Add a row to this TEMPLATE_MAP
